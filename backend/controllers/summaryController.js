...
const getHighlightDates = async (req, res) => {
    try {
        ...
        const query = `
            SELECT DISTINCT date(si.date_created AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Manila') as date
            FROM sales_items SI 
            JOIN sales S on SI.sale_id = S.sale_id
            WHERE S.status = true
            UNION
            SELECT DISTINCT date(sm.date_created AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Manila') as date
            FROM sales_mechanics SM
            JOIN sales S on SM.sale_id = S.sale_id
            WHERE S.status = true
            UNION
            SELECT DISTINCT date(date_created AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Manila') as date
            FROM expenses
            ORDER BY date DESC;
        `
...
const getReceiptItems = async (req, res) => {
    try {
        ...
        const query = `
            SELECT I.item_name as item_name, SI.item_qty as qty, SI.item_unit_price as item_unit_price , SI.item_total_price as item_total_price, 'item' as record_type, SI.refund_qty as refund_qty
            FROM sales_items SI 
            JOIN items I ON SI.item_id = I.item_id 
            WHERE SI.sale_id = $1
                UNION
            SELECT M.mechanic_name as item_name, '1' as qty, SM.service_price as item_unit_price, SM.service_price as item_total_price, 'mechanic' as record_type, '0' as refund_qty
            FROM sales_mechanics SM 
            JOIN mechanics M ON SM.mechanic_id = M.mechanic_id 
            WHERE SM.sale_id = $1
