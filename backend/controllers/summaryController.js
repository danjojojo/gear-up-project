...
const getHighlightDates = async (req, res) => {
    try {
        ...
        const query = `
            SELECT DISTINCT date(si.date_created) as date, 'item' as record_type
            FROM sales_items SI 
            JOIN sales S on SI.sale_id = S.sale_id
            WHERE S.status = true
            UNION
            SELECT DISTINCT date(sm.date_created) as date, 'mechanic' as record_type
            FROM sales_mechanics SM
            JOIN sales S on SM.sale_id = S.sale_id
            WHERE S.status = true
            UNION
            SELECT DISTINCT date(date_created) as date, 'expense' as record_type
            FROM expenses
            ORDER BY date DESC;
        `
