...
const getSummaryRecords = async (req, res) => {
    try {
        ...
        console.log( date);
            ...
            const query = `
                SELECT 'items' as record_type, item_name, item_total_price, si.date_created as date, pos_name, s.sale_id as record_id, si.sale_item_id as record_item_id
                    FROM sales_items SI 
...
const getReceiptItems = async (req, res) => {
    try {
        ...
        const query = `
            SELECT I.item_name as item_name, SI.item_qty as qty, SI.item_unit_price as item_unit_price , SI.item_total_price as item_total_price, 'item' as record_type
            FROM sales_items SI 
            JOIN items I ON SI.item_id = I.item_id 
            WHERE SI.sale_id = $1
                UNION
            SELECT M.mechanic_name as item_name, '1' as qty, SM.service_price as item_unit_price, SM.service_price as item_total_price, 'mechanic' as record_type
            FROM sales_mechanics SM 
            JOIN mechanics M ON SM.mechanic_id = M.mechanic_id 
            WHERE SM.sale_id = $1
